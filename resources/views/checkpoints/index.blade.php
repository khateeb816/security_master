        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            ...
            <div class="d-flex justify-content-end align-items-center mb-4 gap-2 flex-wrap">
                <form method="GET" class="d-flex align-items-center gap-2 flex-wrap mb-0" style="max-width: 700px;">
                    <div class="input-group">
                        <select name="client_id" id="clientFilter" class="form-select" style="min-width: 180px; border-radius: 8px 0 0 8px;">
                            ...
                            @foreach ($clients as $client)
                                <option value="{{ $client->id }}" {{ request('client_id', $selectedClient) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-group">
                        <select name="branch_id" id="branchFilter" class="form-select" style="min-width: 160px; border-radius: 8px 0 0 8px;" {{ !request('client_id', $selectedClient) ? 'disabled' : '' }}>
                            <option value="">Select Branch</option>
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" {{ request('branch_id', $selectedBranch) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <tbody id="checkpointsTableBody">
                            @if (isset($checkpoints) && $checkpoints->count() > 0)
                                @foreach ($checkpoints as $index => $checkpoint)
                                    <tr data-id="{{ $checkpoint->id }}" data-branch-id="{{ $checkpoint->branch_id }}"
                                        data-client-id="{{ $checkpoint->client_id }}">
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $checkpoint->name }}</td>
                                        <td>{{ $checkpoint->branch && $checkpoint->branch->client ? $checkpoint->branch->client->name : '-' }}
                                        </td>
                                        <td>{{ $checkpoint->branch ? $checkpoint->branch->name : '-' }}</td>
                                        <td>
                                            <span
                                        ...
                                        <td class="text-nowrap">
                                            <div class="d-flex gap-1">
                                                ...
                                                <form action="{{ route('clients.branches.checkpoints.destroy', [
                                                    'client' => $checkpoint->client_id,
                                                    'branch' => $checkpoint->branch_id,
                                                    'checkpoint' => $checkpoint->id,
                                                ...
                                                <button
                                                    ...
                                                    data-radius="{{ $checkpoint->radius }}" title="Show on Map">

        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                    ...
                    <a href="<?php echo e(route('guards.index')); ?>" class="btn btn-light btn-sm">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-8"><?php echo e($guard->name); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">Email:</div>
                                <div class="col-sm-8"><?php echo e($guard->email); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">Phone:</div>
                                <div class="col-sm-8"><?php echo e($guard->phone ?? 'N/A'); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">Status:</div>
                                <div class="col-sm-8">
                                    <span class="badge bg-<?php echo e($guard->status === 'active' ? 'success' : 'danger'); ?>">
                                        <?php echo e(ucfirst($guard->status ?? 'inactive')); ?>
                        ...
                        <div class="col-md-6">
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-8"><?php echo e($guard->address ?? 'N/A'); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">City:</div>
                                <div class="col-sm-8"><?php echo e($guard->city ?? 'N/A'); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">Country:</div>
                                <div class="col-sm-8"><?php echo e($guard->country ?? 'N/A'); ?></div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-4 fw-bold">Language:</div>
                                <div class="col-sm-8"><?php echo e($guard->language ?? 'N/A'); ?></div>
                            </div>
                        </div>
                    </div>
                    <?php if($guard->notes): ?>
                    <div class="row">
                        <div class="col-12">
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10"><?php echo e($guard->notes); ?></div>
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $assignedCheckpoints; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $assignment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr>
                                    <td><?php echo e($loop->iteration); ?></td>
                                    <td><?php echo e($assignment->checkpoint->name); ?></td>
                                    <td><?php echo e($assignment->checkpoint->client->name ?? 'N/A'); ?></td>
                                    <td><?php echo e($assignment->checkpoint->branch->name ?? 'N/A'); ?></td>
                                    <td><?php echo e($assignment->date_to_check ? \Carbon\Carbon::parse($assignment->date_to_check)->format('M d, Y') : 'N/A'); ?></td>
                                    <td><?php echo e($assignment->time_to_check ? \Carbon\Carbon::parse($assignment->time_to_check)->format('H:i') : 'N/A'); ?></td>
                                    <td>
                                        <span class="badge bg-<?php echo e($assignment->status === 'completed' ? 'success' : ($assignment->status === 'pending' ? 'warning' : 'secondary')); ?>">
                                            <?php echo e(ucfirst($assignment->status ?? 'pending')); ?>

                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge bg-<?php echo e($assignment->priority === 'high' ? 'danger' : ($assignment->priority === 'medium' ? 'warning' : 'info')); ?>">
                                            <?php echo e(ucfirst($assignment->priority ?? 'normal')); ?>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <button class="btn btn-warning btn-sm edit-assignment-btn"
                                                    ...
                                                    data-checkpoint-id="<?php echo e($assignment->checkpoint_id); ?>"
                                                    data-client-id="<?php echo e($assignment->checkpoint->client->id); ?>"
                                                    data-branch-id="<?php echo e($assignment->checkpoint->branch->id); ?>"
                                                    data-priority="<?php echo e($assignment->priority); ?>"
                                                    data-date="<?php echo e($assignment->date_to_check); ?>"
                                                    data-time="<?php echo e($assignment->time_to_check); ?>"
                                                    data-notes="<?php echo e($assignment->notes); ?>"
                                            >
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form action="<?php echo e(route('guards.removeAssignment', $assignment->id)); ?>" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to remove this assignment?');">
                                                <?php echo csrf_field(); ?>
...
<div class="modal fade" id="assignCheckpointModal" tabindex="-1" aria-labelledby="assignCheckpointModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="assignCheckpointForm" class="modal-content" method="POST" action="<?php echo e(route('guards.assignCheckpoint')); ?>">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="guard_id" value="<?php echo e($guard->id); ?>">
            <input type="hidden" name="assignment_id" id="assignment_id">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title" id="assignCheckpointModalLabel">
                    <i class="fas fa-map-marker-alt me-1"></i> Assign Checkpoint to <?php echo e($guard->name); ?>
            ...
            success: function(branchResponse) {
                ...
                $.ajax({
                    ...
                    success: function(checkpointResponse) {
                        ...
                        $('#assignCheckpointModalLabel').html('<i class="fas fa-edit me-1"></i> Edit Assignment for <?php echo e($guard->name); ?>');
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH K:\Laravel\security-master\resources\views/guards/show.blade.php ENDPATH**/ ?>